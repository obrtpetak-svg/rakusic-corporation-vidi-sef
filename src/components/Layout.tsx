...
const LazyFallback = ({ type = 'dashboard' }) => {
    if (type === 'table') return (
        <div style={{ animation: 'fadeIn 0.3s ease', padding: '4px 0' }}>
            {/* Header */}
            ...
            {/* Table header */}
            <div style={{ display: 'flex', gap: 16, padding: '10px 16px', borderBottom: '1px solid var(--border)' }}>
                {[60, 100, 80, 70, 50, 90].map((w, i) => <SkeletonBlock key={i} w={w} h={10} r={4} />)}
            </div>
            {/* Table rows */}
            {[0, 1, 2, 3, 4, 5, 6].map(i => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '14px 16px', borderBottom: '1px solid var(--divider)' }}>
                ...
                {[0, 1, 2, 3, 4, 5].map(i => (
                    <div key={i} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 12, padding: 20 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 12 }}>
                            ...
                            <div style={{ flex: 1 }}><SkeletonBlock w={120} h={14} r={4} /><SkeletonBlock w={80} h={12} r={4} style={{ marginTop: 6 }} /></div>
                        </div>
    ...
    return (
        <div style={{ animation: 'fadeIn 0.3s ease', padding: '4px 0' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 16, marginBottom: 24 }}>
                {[0, 1, 2, 3].map(i => (
                    <div key={i} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 16, padding: 20, display: 'flex', alignItems: 'center', gap: 14 }}>
                        <SkeletonBlock w={48} h={48} r={14} />
                        <div style={{ flex: 1 }}><SkeletonBlock w={60} h={10} /><SkeletonBlock w={80} h={24} style={{ marginTop: 6 }} /><SkeletonBlock w={50} h={10} style={{ marginTop: 4 }} /></div>
                    </div>
                ))}
            </div>
            {/* Content cards skeleton */}
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 20 }}>
                <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 16, padding: 20, minHeight: 200 }}>
