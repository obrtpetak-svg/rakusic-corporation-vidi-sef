...
import { LazyFallback } from './layout/LazyFallback';
...
import { NotificationPrompt } from './layout/NotificationPrompt';
    ...
    return (
      ...
      `}</style>
            ...
            {showOnboarding && (
                <OnboardingTour
                    haptic={haptic}
                    onComplete={() => { setShowOnboarding(false); localStorage.setItem('vidisef-onboarded', '1'); }}
                />
            )}
            <PwaInstallBanner />
            {
                notifPermission !== 'granted' && notifPermission !== 'denied' && (
                    <NotificationPrompt onEnable={enableNotifications} onDismiss={() => setNotifPermission('denied')} />
                )
            }
