            ...
            {!isWorker && (
                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(4, 1fr)', gap: 12, marginBottom: 20 }}>
                    <div style={{ ...styles.card, display: 'flex', alignItems: 'center', gap: 14 }}>
                        ...
                        <div><div className="u-stat-label">Ukupno</div><div className="u-fs-24 u-fw-800" style={{ color: C.text }}>{totalLogs}</div></div>
                    </div>
                    <div style={{ ...styles.card, display: 'flex', alignItems: 'center', gap: 14 }}>
                        <div style={{ background: 'rgba(59,130,246,0.12)', borderRadius: 12, padding: 12, color: '#2563EB' }}><Icon name="calendar" size={22} /></div>
                        <div><div className="u-stat-label">Ovaj tjedan</div><div className="u-fs-24 u-fw-800" style={{ color: C.text }}>{logsThisWeek}</div></div>
                    </div>
                    <div style={{ ...styles.card, display: 'flex', alignItems: 'center', gap: 14 }}>
                        <div style={{ background: 'rgba(34,197,94,0.12)', borderRadius: 12, padding: 12, color: C.green }}><Icon name="eye" size={22} /></div>
                        <div><div className="u-stat-label">S fotografijama</div><div className="u-fs-24 u-fw-800" style={{ color: C.text }}>{logsWithPhotos}</div></div>
                    </div>
                    <div style={{ ...styles.card, display: 'flex', alignItems: 'center', gap: 14 }}>
                        <div style={{ background: 'rgba(234,179,8,0.12)', borderRadius: 12, padding: 12, color: C.yellow }}>☀️</div>
                        <div><div className="u-stat-label">Najčešće vrijeme</div><div style={{ fontSize: 16, fontWeight: 800, color: C.text }}>{weatherOptions.reduce((a, b) => (logsByWeather[a] || 0) >= (logsByWeather[b] || 0) ? a : b, 'sunčano')}</div></div>
                    </div>
                </div>
            )}
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 8 }}>
                                <div style={{ flex: 1, minWidth: 200 }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                                        ...
                                        <span className="u-fs-12" style={{ color: C.textMuted }}>•</span>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6, flexWrap: 'wrap' }}>
                                        ...
                                        {log.workersPresent && <span className="u-fs-12" style={{ color: C.textMuted }}>👷 {log.workersPresent} radnika</span>}
                                        {photoCount > 0 && <span className="u-fs-12" style={{ color: C.textMuted }}>📷 {photoCount} foto</span>}
                ...
                <Modal title={editId ? 'Uredi dnevnik' : 'Novi dnevnik gradilišta'} onClose={() => setShowForm(false)} wide>
                    ...
                    {(weatherSuggestion || weatherLoading) && (
                        <div style={{ margin: '12px 0', padding: '12px 16px', borderRadius: 12, background: 'linear-gradient(135deg, rgba(59,130,246,0.06), rgba(249,115,22,0.06))', border: '1px solid rgba(59,130,246,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
                            {weatherLoading ? (
                                <span className="u-fs-13" style={{ color: C.textMuted }}>⏳ Učitavam vremenske uvjete...</span>
                            ) : weatherSuggestion && (
                                <>
