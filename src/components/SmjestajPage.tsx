    ...
    return (
        <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
                <div style={{ fontSize: 22, fontWeight: 800, color: C.text }}>{isWorker ? 'Moj smještaj' : 'Smještaj'}</div>
                    ...
                    return (
                        <div key={s.id} style={styles.card}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                                <div><div style={{ fontSize: 15, fontWeight: 700, color: C.text }}>{s.name}</div><div style={{ fontSize: 12, color: C.textMuted }}>{s.address && `📍 ${s.address}`}{s.city && `, ${s.city}`}</div></div>
