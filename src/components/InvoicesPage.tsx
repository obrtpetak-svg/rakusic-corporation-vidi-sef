        ...
        <div>
            ...
            {!isWorker && invoices.filter(i => i.status === 'na čekanju' && i.source === 'radnik').length > 0 && (
                <div style={{ ...styles.card, background: 'rgba(234,179,8,0.08)', borderColor: 'rgba(234,179,8,0.3)', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <div><div style={{ fontWeight: 700, color: C.yellow, fontSize: 14 }}>{invoices.filter(i => i.status === 'na čekanju' && i.source === 'radnik').length} računa čeka odobrenje</div><div style={{ fontSize: 12, color: C.textMuted }}>Računi poslani od radnika</div></div>
                </div>
            )}
